...
function getBasket() {
    $session_id = session_id();
    $sql = "SELECT basket.id as basket_id, catalog.id as good_id, catalog.name as name, catalog.price as price, catalog.image as image FROM `basket`, `catalog` WHERE basket.goods_id=catalog.id AND `session_id`='{$session_id}'";
